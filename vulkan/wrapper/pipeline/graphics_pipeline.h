...
// Configured Vk structs come here.
struct GraphicsPipelineInput {
  VkPipelineVertexInputStateCreateInfo vertexInputInfo;
  lib::Buffer<VkPipelineShaderStageCreateInfo> shaders;
  lib::Buffer<VkDescriptorSetLayout> vkDescriptorSetLayouts;
  lib::Buffer<VkPushConstantRange> vkPushConstantRanges;
};

// Parameters which modify and tune the pipeline creation.
...
public:
  GraphicsPipeline(const Renderpass& renderpass, const ShaderProgram& shaderProgram,
    ...
    const lib::Buffer<VkDescriptorSetLayout> vkDescriptorSetLayouts =
        shaderProgram.getVkDescriptorSetLayouts();
    std::span<const VkPushConstantRange> vkPushConstantRanges = shaderProgram.getPushConstants();
    ...
    lib::Buffer<VkPipelineShaderStageCreateInfo> shaders =
        shaderProgram.getVkPipelineShaderStageCreateInfos();
  ...
  GraphicsPipeline(
      ...
      std::span<VkPipelineShaderStageCreateInfo> shaders, std::span<const VkDescriptorSetLayout>
          vkDescriptorSetLayouts,
      std::span<const VkPushConstantRange> vkPushConstantRanges,
                   const GraphicsPipelineParameters& parameters)
    : Pipeline(VK_PIPELINE_BIND_POINT_GRAPHICS), _renderpass(renderpass) {
    const VkDevice device = _renderpass.getLogicalDevice().getVkDevice();
