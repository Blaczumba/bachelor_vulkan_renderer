...
std::unique_ptr<common::Presentation> Presentation::create(const AndroidAppState& androidAppState, common::GraphicsApi graphicsApi, const FileLoader& fileLoader) {
  ...
  std::unique_ptr<common::GraphicsContext> graphicsContext = graphicsPlugin->createGraphicsContext(instance->getXrInstance(), system->getXrSystemId(), fileLoader);
  std::unique_ptr<Session> session = Session::create(*system, *graphicsPlugin);
  std::vector<Swapchain> swapchains = SwapchainBuilder()
      .withArraySize(2)
  ...
  graphicsContext->createPresentingResources(graphicsPlugin->getSwapchainContext(swapchain.getSwapchain()));
  return nullptr;
}
